...
OPS_TEMPLATE = (
    ...
    f"  @{BOT_NAME} query gid t-0002        按 task_id 精确查询\n"
    f"  @{BOT_NAME} query label 三国杀      按 label 模糊查询\n"
    f"  @{BOT_NAME} query password          查看 .bin 解压密码\n"
    "\n"
    "【导出】\n"
    f"  @{BOT_NAME} export table all        导出 filename 去重表（全部状态）\n"
    f"  @{BOT_NAME} export table success    导出指定状态的 filename 去重表\n"
    "  字段：filename / label / updated_at / finished_at\n"
    "\n"
    "【帮助】help / ?    【打招呼】你好 / hi\n"
    "\n"
    "【回传说明】\n"
